...
template <typename T>
__global__ void ImpreBitonic_TopKLocalSortInPlace(T* __restrict__ in, T* __restrict__ out,
                                                  const int k, const int klog2, const int impre_pe) {
    // const int k = K;
...
cudaError_t impreciseBitonicTopK(KeyT* d_keys_in, unsigned int num_items, unsigned int k, KeyT* d_keys_out,
    ...
    if (num_items >= (wg_size << NUM_ELEM_BITSHIFT << impre_pe_shift)) {
        ...
        ImpreBitonic_TopKLocalSortInPlace<KeyT><<<numThreads / wg_size, wg_size, share_mem_size>>>(d_keys.Current(), d_keys.Alternate(), k, klog2, impre_pe);
