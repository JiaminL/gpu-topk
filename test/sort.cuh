...
cudaError_t sort(KeyT *d_keys_in, unsigned int num_items, CachingDeviceAllocator &g_allocator) {
    ...
    if (d_keys.Current() != d_keys_in) 
        CubDebugExit(cudaMemcpy(d_keys_in, d_keys.Current(), sizeof(KeyT) * num_items, cudaMemcpyDeviceToDevice));
